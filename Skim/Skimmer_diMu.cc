...
void Skimmer::Loop(TString outputName, int skm)
{
    ...
    for (int jentry=0; jentry<nentries;jentry++) {
        ...
        TLorentzVector Mu4Momentum,JMu4Momentum, Jet4Momentum;
        ...
        for (int imu = 0; imu < nMu; ++imu){
            if (muPt->at(imu) > 30 && (muIDbit->at(imu) >> 2 & 1) && fabs(muEta->at(imu)) < 2.4){
                Mu4Momentum.SetPtEtaPhiM(muPt->at(imu),muEta->at(imu),muPhi->at(imu),MuMass);
                
                for (int jmu = 0; jmu < nMu; ++jmu){
                    if (muPt->at(jmu) > 30 && (muIDbit->at(jmu) >> 2 & 1) && fabs(muEta->at(jmu)) < 2.4){
                        JMu4Momentum.SetPtEtaPhiM(muPt->at(jmu),muEta->at(jmu),muPhi->at(jmu),MuMass);
                        
                        
                        for (int ijet = 0; ijet < nJet; ++ijet){
                            Jet4Momentum.SetPtEtaPhiM(jetPt->at(ijet),jetEta->at(ijet),jetPhi->at(ijet),jetEn->at(ijet));
                            if (jetPt->at(ijet) > 100 &&  fabs(jetEta->at(ijet)) < 3.0   && Jet4Momentum.DeltaR(Mu4Momentum) > 0.5  && Jet4Momentum.DeltaR(JMu4Momentum) > 0.5  && JMu4Momentum.DeltaR(Mu4Momentum) > 0.5  ){
                                isMuJet=1;
                                break;
