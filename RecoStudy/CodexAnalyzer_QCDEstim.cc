        ...
        for (Int_t i = 0; i < nentries_wtn; i++) {
            ...
            for  (int jmu=0 ; jmu < nMu; jmu++){
                ...
                if ( (muPFNeuIso->at(jmu) + muPFPhoIso->at(jmu) - 0.5* muPFPUIso->at(jmu) )  > 0.0)
                    IsoMu= ( muPFChIso->at(jmu)/muPt->at(jmu) + muPFNeuIso->at(jmu) + muPFPhoIso->at(jmu) - 0.5* muPFPUIso->at(jmu))/muPt->at(jmu);
                
                bool MuPtCut = muPt->at(jmu) > 20 && fabs(muEta->at(jmu)) < 2.4 ;
                bool MuIdIso=(muIsLooseID->at(jmu) > 0  && fabs(muD0->at(jmu)) < 0.045 && fabs(muDz->at(jmu)) < 0.2 && IsoMu < 0.25 );
                
                if ( MuPtCut  && MuIdIso ) secondMuveto++;
            }
            
            
            if ((numTau+numElectron + numBJet+numZboson) > 0) continue;
            if (secondMuveto > 1) continue;
