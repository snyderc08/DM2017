...
#    ######  Add OverFlow Bin
#    if not FileName.find("_tmass_EleMet") > 0:
...
#
#    if FileName.find("tmass_EleMet") > 0 :
#        print "##################################\n", FileName
#        for i in range(Data.GetNbinsX()):
#                if i > 9 : Data.SetBinContent(i+1,0)
...
#    legende.AddEntry(QCD,"QCD multijet","f")
    ...
    #       if i==1 or i==3: 
    categ.AddText(scale.replace("TotalRoot_DiEle_","").replace("EleJet",""))
    ...
    c.Modified()
    c.SaveAs("_plot_DiEle"+scale+FileName.replace('TotalRootForLimit_PreSelection_DiEle_EleJet','').replace('.root','')+str(isLOG)+".pdf")
...
#               ["_dPhi_Ele_Met","#Delta#phi (#mu,MET)","",5,1],
#               ["_dPhi_Ele_Jet","#Delta#phi (#mu,jet)","",5,1],
...
#Scales=["TotalRoot_DiEle_ScaledWZEleJet","TotalRoot_DiEle_NoZScaleEleJet","TotalRoot_DiEle_RealDiEleEleJet"]
Scales=["TotalRoot_DiEleEleJet"]
            ...
            for jpt in JPT:
                for isLOG in logStat:
                    for scale in Scales:
                        ...
                        print "---->", FileName
                        MakePlot(FileName,"EleJet","",axisName,Info,Bin,"",yMin,isLOG,scale,mt,Binning)
