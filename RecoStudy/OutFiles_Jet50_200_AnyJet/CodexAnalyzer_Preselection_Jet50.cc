...
//    std::string ROOTLocHT= "/Users/abdollah1/GIT_abdollah110/DM2017/ROOT80X/SampleLQ2/";
    std::string ROOTLocHT= "/Users/abdollah1/GIT_abdollah110/DM2017/ROOT80X/lqsamples2_jetPt50_MT300/";
...
//                }
                ...
                for (int ijet= 0 ; ijet < nJet ; ijet++){
                    ...
                    bool goodJet = (jetPFLooseId->at(ijet) > 0.5 && jetPt->at(ijet) > JetPtCut && jetPt->at(ijet) < 200 && fabs(jetEta->at(ijet)) < 2.4 && Jet4Momentum.DeltaR(Mu4Momentum) > 0.5);
                    if (! goodJet) continue;
...
//                    std::string MT_Cat[size_mTCat] = {"_NoMT","_HighMT","_MT50To150","_MT150to200","_MT200to250","_MT250to300","_MT300to350","_MT200","_MT300","_MT400","_MT500"};
                    bool MT_category[size_mTCat] = {NoMT,HighMT,MTMore300,MTMore500};
                    std::string MT_Cat[size_mTCat] = {"_NoMT","_HighMT","_MT300","_MT500"};
...
//                                                            if (nMu > 1) plotFill(CHL+"_dPhi_Mu2_Met"+FullStringName,deltaPhi(Mu24Momentum.Phi(),pfMETPhi),160,0,3.2,FullWeight);
                                                            ...
                                                            plotFill(CHL+"_ST"+FullStringName,recoHT+muPt->at(imu),300,0,3000,FullWeight);
                                                                    if (ST < 110) cout<< recoHT+muPt->at(imu) << "  st="<<ST << "  recoHT="<<recoHT << "  lepPt="<<muPt->at(imu)<< " iso="<<IsoMu<< "jetpt="<<jetPt->at(ijet)<< "  ijet=" <<ijet<<"\n";
