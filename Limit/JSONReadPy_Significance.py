...
FinalSampleDirectory='Sig_LIMITS.json'
...
OurFile=TFile('out.root','RECREATE')
can=TCanvas("","",600,600)
OutHist=TH2F('Sig','',8,750,1550,9,275,725)
...
#        print 'hlimitxsec2->SetBinContent(%d,%d,%f); // Bin_%d_LQ_%d_DM_%d_X_%d'%(XbinLQ,YbinDM,data[str(M[0])]["exp0"],M[0], M[1], M[2], M[3])
        print 'hlimitxsec2->SetBinContent(%d,%d,%f); // Bin_%d_LQ_%d_DM_%d_X_%d'%(XbinLQ,YbinDM,data[str(M[0])]["obs"]/(LQXS(M[1])* 1000 * 1),M[0], M[1], M[2], M[3])
        OutHist.SetBinContent(XbinLQ,YbinDM,data[str(M[0])]["obs"])
